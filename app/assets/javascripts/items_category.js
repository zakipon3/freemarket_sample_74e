$(function(){
  ...
  if(request.indexOf("new") != -1){
    $.ajax({
      ...
      $("#category-select").append(`<select class="exhibit__wrapper__itemdetail__selectbox select-parent" name="item[category_id]" id="item_category_id"><option value="">選択してください</option></select>`);
      ...
      $(".select-parent").on("change", function(){
        ...
        if($(this).val() == ""){
          ...
          $.ajax({
            ...
            $("#category-select").append(`<select class="exhibit__wrapper__itemdetail__selectbox select-child" name="item[category_id]" id="item_category_id"><option value="">選択してください</option></select>`);
      ...
      $("#category-select").on("change", ".select-child", function(){
        ...
        if($(this).val() == ""){
          ...
          $.ajax({
            ...
            $("#category-select").append(`<select class="exhibit__wrapper__itemdetail__selectbox select-grandchild" name="item[category_id]" id="item_category_id"><option value="">選択してください</option></select>`);
  ...
  } else if(request.indexOf("edit") != -1){
    $.ajax({
      url: "/items/set_parents"
    }).done(function(data){
      // console.log("11");
      ...
      $(".select-parent").on("change", function(){
        ...
        if($(this).val() == ""){
          ...
          $.ajax({
            ...
            $(".select-parent").attr("name", "select-parent");
            $(".select-parent").css("margin-bottom", "10px");
            $("#category-edit").append(`<select class="exhibit__wrapper__itemdetail__selectbox select-child" name="item[category_id]" id="item_category_id"><option value="">選択してください</option></select>`);
            data.children.forEach(function(child){
              $(".select-child").append(`<option value="${child.id}">${child.name}</option>`);
            })
          })
      ...
      $("#category-edit").on("change", ".select-child", function(){
        ...
        if($(this).val() == ""){
          ...
          $.ajax({
            ...
            $(".select-child").attr("name", "select-parent");
            $(".select-child").css("margin-bottom", "10px");
            $("#category-edit").append(`<select class="exhibit__wrapper__itemdetail__selectbox select-grandchild" name="item[category_id]" id="item_category_id"><option value="">選択してください</option></select>`);
            data.grandchildren.forEach(function(grandchild){
              $(".select-grandchild").append(`<option value="${grandchild.id}">${grandchild.name}</option>`);
            })
          })
